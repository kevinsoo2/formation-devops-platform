...
import { extraModules2 } from './seed-data-extra2.js';
import { extraModules3 } from './seed-data-extra3.js';
...
export const seedData = {
  ...
  modules: [..._seedData.modules, ...v3Modules, ...extraModules, ...extraModules2, ...extraModules3],
  quizQuestions: [
    ..._seedData.quizQuestions,
    // ==================== DOCKER QUIZ ====================
    { id: 'quiz-docker-01', courseId: 'docker', moduleId: 'docker-01',
      question: 'Quelle est la différence principale entre un container et une machine virtuelle ?',
      options: JSON.stringify(['Le container virtualise le hardware', 'Le container partage le kernel du host sans OS complet', 'Le container est plus lent qu\'une VM', 'Il n\'y a aucune différence fondamentale']),
      correctIndex: 1,
      explanation: 'Un container partage le kernel de l\'hôte et virtualise uniquement l\'espace utilisateur, contrairement à une VM qui virtualise tout le hardware avec un OS complet. Cela rend les containers beaucoup plus légers.' },
    { id: 'quiz-docker-02', courseId: 'docker', moduleId: 'docker-01',
      question: 'Quel est l\'avantage principal d\'un multi-stage build dans un Dockerfile ?',
      options: JSON.stringify(['Accélérer le temps de build', 'Réduire la taille de l\'image finale en séparant build et runtime', 'Permettre l\'exécution de plusieurs containers', 'Gérer automatiquement les dépendances']),
      correctIndex: 1,
      explanation: 'Le multi-stage build permet de séparer l\'étape de compilation (avec tous les outils de build) de l\'image finale qui ne contient que le binaire et ses dépendances runtime, réduisant drastiquement la taille.' },
    { id: 'quiz-docker-03', courseId: 'docker', moduleId: 'docker-01',
      question: 'Quelle commande Docker Compose permet de lancer tous les services en arrière-plan ?',
      options: JSON.stringify(['docker compose start', 'docker compose up -d', 'docker compose run --detach', 'docker compose deploy']),
      correctIndex: 1,
      explanation: 'docker compose up -d (detached mode) lance tous les services définis dans le docker-compose.yml en arrière-plan, créant les réseaux et volumes si nécessaire.' },

    // ==================== KUBERNETES QUIZ ====================
    { id: 'quiz-kubernetes-01', courseId: 'kubernetes', moduleId: 'kubernetes-01',
      question: 'Qu\'est-ce qu\'un Pod dans Kubernetes ?',
      options: JSON.stringify(['Un serveur physique du cluster', 'La plus petite unité déployable, contenant un ou plusieurs containers', 'Un fichier de configuration YAML', 'Un service réseau']),
      correctIndex: 1,
      explanation: 'Un Pod est la plus petite unité déployable dans Kubernetes. Il encapsule un ou plusieurs containers qui partagent le même réseau, stockage et spécification de cycle de vie.' },
    { id: 'quiz-kubernetes-02', courseId: 'kubernetes', moduleId: 'kubernetes-01',
      question: 'Quel type de Service Kubernetes expose une application à l\'extérieur du cluster ?',
      options: JSON.stringify(['ClusterIP', 'NodePort ou LoadBalancer', 'ExternalName', 'HeadlessService']),
      correctIndex: 1,
      explanation: 'NodePort expose le service sur un port de chaque noeud du cluster, et LoadBalancer provisionne un load balancer externe (cloud). ClusterIP n\'est accessible que depuis l\'intérieur du cluster.' },
    { id: 'quiz-kubernetes-03', courseId: 'kubernetes', moduleId: 'kubernetes-01',
      question: 'Quel objet Kubernetes assure qu\'un nombre spécifié de réplicas d\'un Pod est toujours en cours d\'exécution ?',
      options: JSON.stringify(['Service', 'ConfigMap', 'Deployment (via ReplicaSet)', 'Ingress']),
      correctIndex: 2,
      explanation: 'Un Deployment gère un ReplicaSet qui maintient le nombre souhaité de réplicas de Pod en fonctionnement. Si un Pod tombe, le ReplicaSet en recrée un automatiquement.' },

    // ==================== TERRAFORM QUIZ ====================
    { id: 'quiz-terraform-01', courseId: 'terraform', moduleId: 'terraform-01',
      question: 'Quel est le langage de configuration utilisé par Terraform ?',
      options: JSON.stringify(['YAML', 'JSON uniquement', 'HCL (HashiCorp Configuration Language)', 'Python']),
      correctIndex: 2,
      explanation: 'Terraform utilise HCL (HashiCorp Configuration Language), un langage déclaratif conçu pour être lisible par les humains tout en étant structuré pour la machine. Il supporte aussi JSON comme alternative.' },
    { id: 'quiz-terraform-02', courseId: 'terraform', moduleId: 'terraform-01',
      question: 'Quel est le rôle du fichier terraform.tfstate ?',
      options: JSON.stringify(['Stocker les variables d\'environnement', 'Mapper les ressources déclarées à leurs identifiants réels dans le cloud', 'Configurer les providers', 'Définir les outputs du projet']),
      correctIndex: 1,
      explanation: 'Le fichier state (terraform.tfstate) est la source de vérité de Terraform. Il mappe chaque ressource déclarée dans le code à son identifiant réel dans l\'infrastructure, permettant de détecter les drifts et planifier les changements.' },
    { id: 'quiz-terraform-03', courseId: 'terraform', moduleId: 'terraform-01',
      question: 'Quel est l\'avantage principal des modules Terraform ?',
      options: JSON.stringify(['Accélérer le provisioning', 'Encapsuler et réutiliser des groupes de ressources avec une interface standardisée', 'Remplacer les providers', 'Gérer les secrets automatiquement']),
      correctIndex: 1,
      explanation: 'Les modules Terraform encapsulent un ensemble de ressources avec des variables d\'entrée et des outputs, permettant la réutilisation, la standardisation et la composition d\'infrastructure complexe.' },

    // ==================== ANSIBLE QUIZ ====================
    { id: 'quiz-ansible-01', courseId: 'ansible', moduleId: 'ansible-01',
      question: 'Qu\'est-ce qu\'un playbook Ansible ?',
      options: JSON.stringify(['Un fichier binaire exécutable', 'Un fichier YAML décrivant l\'état souhaité des systèmes à configurer', 'Un script shell amélioré', 'Une image Docker de configuration']),
      correctIndex: 1,
      explanation: 'Un playbook Ansible est un fichier YAML qui décrit un ensemble de tâches (plays) à exécuter sur des hôtes cibles. Il définit l\'état souhaité de manière déclarative et ordonnée.' },
    { id: 'quiz-ansible-02', courseId: 'ansible', moduleId: 'ansible-01',
      question: 'Que signifie l\'idempotence dans le contexte d\'Ansible ?',
      options: JSON.stringify(['Exécuter les tâches en parallèle', 'Relancer un playbook produit le même résultat sans effets secondaires si l\'état est déjà correct', 'Chiffrer les données sensibles', 'Gérer plusieurs inventaires simultanément']),
      correctIndex: 1,
      explanation: 'L\'idempotence garantit qu\'exécuter un playbook plusieurs fois produit toujours le même résultat final. Si le système est déjà dans l\'état désiré, Ansible ne fait rien (status: ok au lieu de changed).' },
    { id: 'quiz-ansible-03', courseId: 'ansible', moduleId: 'ansible-01',
      question: 'Quel est l\'avantage des rôles Ansible par rapport aux playbooks monolithiques ?',
      options: JSON.stringify(['Les rôles sont plus rapides à exécuter', 'Les rôles structurent le code en composants réutilisables avec tasks, handlers, vars et templates', 'Les rôles ne nécessitent pas SSH', 'Les rôles remplacent l\'inventaire']),
      correctIndex: 1,
      explanation: 'Les rôles organisent le code Ansible en composants modulaires réutilisables avec une structure standardisée (tasks/, handlers/, vars/, templates/, files/), facilitant le partage et la maintenance.' },

    // ==================== GITLAB CI QUIZ ====================
    { id: 'quiz-gitlab-ci-01', courseId: 'gitlab-ci', moduleId: 'gitlab-ci-01',
      question: 'Quel fichier configure le pipeline CI/CD dans GitLab ?',
      options: JSON.stringify(['Jenkinsfile', '.github/workflows/ci.yml', '.gitlab-ci.yml à la racine du dépôt', 'pipeline.yaml dans .gitlab/']),
      correctIndex: 2,
      explanation: 'Le fichier .gitlab-ci.yml placé à la racine du dépôt est le fichier de configuration standard de GitLab CI/CD. Il définit les stages, jobs, scripts, artifacts et règles du pipeline.' },
    { id: 'quiz-gitlab-ci-02', courseId: 'gitlab-ci', moduleId: 'gitlab-ci-01',
      question: 'Quel est le rôle d\'un Runner GitLab CI ?',
      options: JSON.stringify(['Stocker le code source', 'Exécuter les jobs du pipeline sur une machine (Docker, shell, Kubernetes)', 'Gérer les merge requests', 'Héberger les artifacts']),
      correctIndex: 1,
      explanation: 'Un Runner GitLab CI est un agent qui exécute les jobs définis dans le pipeline. Il peut fonctionner en mode Docker (isolation par container), shell (directement sur la machine) ou Kubernetes.' },
    { id: 'quiz-gitlab-ci-03', courseId: 'gitlab-ci', moduleId: 'gitlab-ci-01',
      question: 'Comment sont organisées les étapes d\'exécution dans un pipeline GitLab CI ?',
      options: JSON.stringify(['Par priorité numérique', 'En stages séquentiels, les jobs d\'un même stage s\'exécutent en parallèle', 'Par ordre alphabétique des noms de jobs', 'Tout s\'exécute en parallèle sans ordre']),
      correctIndex: 1,
      explanation: 'Un pipeline GitLab CI est organisé en stages (build, test, deploy). Les stages s\'exécutent séquentiellement, mais les jobs au sein d\'un même stage s\'exécutent en parallèle, optimisant le temps total.' },

    // ==================== GIT AVANCÉ QUIZ ====================
    { id: 'quiz-git-avance-01', courseId: 'git-avance', moduleId: 'git-avance-01',
      question: 'Quelle est la différence entre git merge et git rebase ?',
      options: JSON.stringify(['Merge est plus rapide que rebase', 'Rebase réécrit l\'historique en rejouant les commits sur une autre base, merge crée un commit de fusion', 'Rebase ne fonctionne que sur la branche main', 'Il n\'y a aucune différence fonctionnelle']),
      correctIndex: 1,
      explanation: 'git rebase réécrit l\'historique en déplaçant les commits d\'une branche sur la pointe d\'une autre, créant un historique linéaire. git merge crée un commit de merge préservant les deux historiques.' },
    { id: 'quiz-git-avance-02', courseId: 'git-avance', moduleId: 'git-avance-01',
      question: 'À quoi sert git cherry-pick ?',
      options: JSON.stringify(['Supprimer un commit de l\'historique', 'Copier un commit spécifique d\'une branche vers la branche courante', 'Créer une nouvelle branche', 'Résoudre les conflits de merge']),
      correctIndex: 1,
      explanation: 'git cherry-pick applique les changements d\'un commit spécifique sur la branche courante, créant un nouveau commit. Utile pour porter un hotfix d\'une branche à une autre sans merge complet.' },
    { id: 'quiz-git-avance-03', courseId: 'git-avance', moduleId: 'git-avance-01',
      question: 'Quel est le rôle des hooks Git ?',
      options: JSON.stringify(['Gérer les branches distantes', 'Exécuter automatiquement des scripts à certains événements Git (pre-commit, pre-push, etc.)', 'Compresser le dépôt', 'Afficher l\'historique graphiquement']),
      correctIndex: 1,
      explanation: 'Les hooks Git sont des scripts exécutés automatiquement lors d\'événements spécifiques (pre-commit, pre-push, post-merge). Ils permettent d\'automatiser la validation du code, le linting, les tests, etc.' },
  ],
};
